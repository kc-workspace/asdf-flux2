...
asdf_download() {
  ...
  local download
  download="https://github.com/fluxcd/flux2/releases/download/v${version}/flux_${version}_${os}_${arch}.tar.gz"
  asdf_info "starting [%15s] %s" \
    "download" "$download"

  local tmpfile="flux_${version}_${os}_${arch}.tar.gz"
  local tmpdir
  tmpdir="$(mktemp -d)"
